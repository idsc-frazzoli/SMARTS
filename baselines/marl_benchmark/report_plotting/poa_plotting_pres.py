...
def main(path):
    ...
    for degree in degrees:
        ...
        ax.bar(locations[1] - 2 * width - 2 * distance -0.02, cost_com_high[degree][6], width, color=PALETTE[0])
        ax.bar(locations[1] - width - distance, cost_per_time_high[degree][6], width, color=PALETTE[1])
        ax.bar(locations[1], cost_per_acc_high[degree][6], width, color=PALETTE[2])
        ax.bar(locations[1] + width + distance, -np.array(rew_per_goal_high[degree][6]), width, color=PALETTE[3])
        ax.bar(locations[1] + 2 * width + 2 * distance +0.02, cost_high[degree][6], width, color="grey")

        ax.bar(locations[0] - 2 * width - 2 * distance-0.02, cost_com_low[degree][6], width, color=PALETTE[0])
        ax.bar(locations[0] - width - distance, cost_per_time_low[degree][6], width, color=PALETTE[1])
        ax.bar(locations[0], cost_per_acc_low[degree][6], width, color=PALETTE[2])
        ax.bar(locations[0] + width + distance, -np.array(rew_per_goal_low[degree][6]), width, color=PALETTE[3])
        ax.bar(locations[0] + 2 * width + 2 * distance+0.02, cost_low[degree][6], width, color="grey")
        ...
        plt.savefig(Path(fig_save_folder, "costs_plot_degree_{}_1_alpha1p36_dist.png".format(degree)), dpi=500)
        # plt.savefig(Path(fig_save_folder, "costs_plot_degree_{}.pdf".format(degree)))
