...
class MultiAgentEnv(Env):
    ...
    def render(self, mode='human'):
        plt.figure(figsize=((self.limits[1] - self.limits[0])/8, 2/8))
        tmp_path = Path('tmp')
        tmp_path.mkdir(parents=True, exist_ok=True)
        plt.plot([self.limits[0], self.limits[1]], [-1, -1], color='k')
        plt.plot([self.limits[0], self.limits[1]], [1, 1], color='k')
        ...
        plt.axis('off')
        plt.savefig(Path(tmp_path, '{0:04}.png'.format(int(self.time_step))), dpi=400)
        plt.close()
